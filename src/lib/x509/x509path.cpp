...
Certificate_Status_Code verify_ocsp_signing_cert(
   ...
   std::vector<X509_Certificate> end_entity_and_supports;
   end_entity_and_supports.push_back(signing_cert);
   end_entity_and_supports.insert(end_entity_and_supports.end(), extra_certs.begin(), extra_certs.end());
   const auto validation_result = x509_path_validate(
                                    end_entity_and_supports,
...
PKIX::check_ocsp(const std::vector<X509_Certificate>& cert_path,
   ...
   for(size_t i = 0; i != cert_path.size() - 1; ++i)
      ...
      X509_Certificate subject = cert_path.at(i);
      X509_Certificate ca = cert_path.at(i+1);
      ...
      if(i < ocsp_responses.size() && (ocsp_responses.at(i) != std::nullopt)
         ...
         try
            ...
            std::vector<X509_Certificate> support_certificates = ocsp_response->certificates();
            support_certificates.insert(support_certificates.end(), cert_path.begin(), cert_path.end());
            ...
            else if(auto ocsp_signing_cert_status = verify_ocsp_signing_cert(signing_cert.value(), ca, support_certificates, certstores, ref_time, restrictions);
