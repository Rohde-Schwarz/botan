         ...
         public:
            ...
            BlockT load_next() const {
               ...
               if constexpr(is_byte_aligned()) {
                  result_block =
                     load_le(std::span{m_source.m_blocks}.subspan(read_bytepos()).template first<block_size>());
               } else {
            ...
            void store_next(BlockT block) {
               ...
               if constexpr(is_byte_aligned()) {
                  auto sink = std::span{m_source.m_blocks}.subspan(write_bytepos()).template first<block_size>();
                  store_le(sink, block);
            ...
            std::span<BlockT> span(size_type blocks) const {
               return {reinterpret_cast<BlockT*>(m_source.m_blocks.data() + read_bytepos()), blocks};
            }

            template <std::unsigned_integral BlockT>
               requires(is_byte_aligned() && is_const())
            std::span<const BlockT> span(size_type blocks) const {
               return {reinterpret_cast<const BlockT*>(m_source.m_blocks.data() + read_bytepos()), blocks};
            }
