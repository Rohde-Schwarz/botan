...
// TODO: We need ML_KEM IPD (rebase on "PQC: ML-KEM (Initial Public Draft)")
...
X_Wing_PrivateKey::X_Wing_PrivateKey(std::span<const uint8_t> key_bytes) :
      X_Wing_PrivateKey([&key_bytes] {
         ...
         auto pk_x_bytes_from_sk = pks[1]->raw_public_key_bits();
...
secure_vector<uint8_t> X_Wing_PrivateKey::private_key_bits() const {
   // X-Wing RFC Draft section 5.2:
   //   sk, pk = concat(sk_M, sk_X, pk_X), concat(pk_M, pk_X)
   secure_vector<uint8_t> bits(SK_LEN);
   BufferStuffer stuffer(bits);
   stuffer.append(private_keys().at(0)->raw_private_key_bits());
   stuffer.append(private_keys().at(1)->raw_private_key_bits());
   stuffer.append(public_keys().at(1)->raw_public_key_bits());
