...
std::vector<uint8_t> rlen(std::span<const uint8_t> s) {
   ...
   const auto be_sz_encoding = store_be<std::array<uint8_t, sizeof(size_t)>>(x);
...
void ounsworth_secret_combiner(std::span<uint8_t> out_shared_secret,
   ...
   // Depending on the KDF, the one-step KDM is called with a Hash function or a MAC (with K).
   std::visit(overloaded{[&](std::unique_ptr<MAC> mac) {
                            Sp800_56C::kdm(out_shared_secret, *mac, kdf_input_buffer, fixed_info, big_k);
                         },
                         [&](std::unique_ptr<HashFunction> hash) {
                            Sp800_56C::kdm(out_shared_secret, *hash, kdf_input_buffer, fixed_info);
                         }},
