...
std::variant<std::unique_ptr<HashFunction>, std::unique_ptr<MAC>> Ounsworth::Mode::kdf_instance(
   size_t output_bytes) const {
   switch(kdf_mode()) {
      case KMAC128:
         return MAC::create_or_throw(fmt("KMAC-128({})", output_bytes * 8));
      case KMAC256:
         return MAC::create_or_throw(fmt("KMAC-256({})", output_bytes * 8));
      case SHA3_256:
