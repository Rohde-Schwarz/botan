...
secure_vector<uint8_t> dom4(uint8_t x, std::span<const uint8_t> y) {
   ...
   secure_vector<uint8_t> res(8 + 1 + 1 + y.size());
   BufferStuffer stuf(res);
   stuf.append(std::array<uint8_t, 8>{'S', 'i', 'g', 'E', 'd', '4', '4', '8'});
   stuf.append(x);
   stuf.append(static_cast<uint8_t>(y.size()));
   stuf.append(y);
...
std::array<uint8_t, 114> hash(bool f, std::span<const uint8_t> context, Ts... xs) {
   auto shake = XOF::create_or_throw("SHAKE-256");
   shake->update(dom4(static_cast<uint8_t>(f), context));
   (shake->update(std::span(xs)), ...);
   std::array<uint8_t, 114> res;
   shake->output(res);
   return res;
}

std::pair<std::array<uint8_t, 57>, std::array<uint8_t, 57>> split(std::span<const uint8_t, 114> arr) {
   std::pair<std::array<uint8_t, 57>, std::array<uint8_t, 57>> res;
   copy_mem(res.first, arr.first<57>());
   copy_mem(res.second, arr.last<57>());
   return res;
}

Scalar448 scalar_from_xof(std::unique_ptr<XOF>& shake) {
...
Ed448Point Ed448Point::decode(std::span<const uint8_t, 57> enc) {
   ...
   if((enc.back() & 0x7F) != 0) {  // last byte is either 0x00 or 0x08
      throw Decoding_Error("Ed448 point has unacceptable x-distinguisher");
   }
   const bool x_distinguisher = enc.back();
   ...
   const auto u = Gf448Elem(y).square() - 1;
   const auto v = d * Gf448Elem(y).square() - 1;
   const auto maybe_x = (u * u.square()) * v * ((u.square().square() * u) * v.square() * v).root();

   // 3. If v * x^2 = u, the recovered x-coordinate is x.  Otherwise, no
   //    square root exists, and the decoding fails.
   if(v * maybe_x.square() != u) {
   ...
   std::array<uint64_t, 7> x_data;
   CT::Mask<uint64_t>::expand(maybe_x_parity == x_distinguisher)
      .select_n(x_data.data(), maybe_x.const_words().data(), (-maybe_x).const_words().data(), 7);

   return {Gf448Elem(x_data), y};
}

Ed448Point Ed448Point::base_point() {
   constexpr std::array<const uint64_t, 7> x = {0x2626a82bc70cc05e,
                                                0x433b80e18b00938e,
                                                0x12ae1af72ab66511,
                                                0xea6de324a3d3a464,
                                                0x9e146570470f1767,
                                                0x221d15a622bf36da,
                                                0x4f1970c66bed0ded};
   constexpr std::array<const uint64_t, 7> y = {0x9808795bf230fa14,
                                                0xfdbd132c4ed7c8ad,
                                                0x3ad3ff1ce67c39c4,
                                                0x87789c1e05a0c2d7,
                                                0x4bea73736ca39840,
...
std::array<uint8_t, 57> Ed448Point::encode() const {
   std::array<uint8_t, 57> res_buf = {0};
...
Ed448Point Ed448Point::double_point() const {
   ...
   const Gf448Elem B = (m_x + m_y).square();
   const Gf448Elem C = m_x.square();
   const Gf448Elem D = m_y.square();
   const Gf448Elem E = C + D;
   const Gf448Elem H = m_z.square();
...
std::array<uint8_t, 57> create_pk_from_sk(std::span<const uint8_t, 57> sk) {
   // 5.2.5. Key Generation
   // The 57-byte public key is generated by the following steps:
   auto shake = XOF::create_or_throw("SHAKE-256");
   shake->update(sk);

   const Scalar448 s = scalar_from_xof(shake);
...
std::array<uint8_t, 114> sign_message(std::span<const uint8_t, 57> sk,
   ...
   const Scalar448 r(hash(pgflag, context, prefix, msg));
   ...
   const Scalar448 k(hash(pgflag, context, big_r, pk, msg));
   ...
   std::array<uint8_t, 114> sig;
   BufferStuffer stuf(sig);
   stuf.append(big_r);
   stuf.append(big_s.to_bytes<57>());
   BOTAN_ASSERT(stuf.full(), "Buffer is full");

   return sig;
}

bool verify_signature(std::span<const uint8_t, 57> pk,
                      bool phflag,
                      std::span<const uint8_t> context,
   ...
   try {
      ...
      const auto [big_r_bytes, big_s_bytes] = split(sig.first<114>());
      ...
      //    interpret the 114-octet digest as a little-endian integer k.
      const Scalar448 k(hash(phflag, context, big_r_bytes, pk, msg));
