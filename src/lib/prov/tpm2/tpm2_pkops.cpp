...
std::vector<uint8_t> Signature_Operation::sign(Botan::RandomNumberGenerator& rng) {
   ...
   auto [digest, validation] = [&] {
      if(auto h = dynamic_cast<HashFunction*>(hash())) {
         ...
         result.first.reset(new TPM2B_DIGEST(init_with_size<TPM2B_DIGEST>(hash()->output_length())));
         result.second.reset(new TPMT_TK_HASHCHECK{
            .tag = TPM2_ST_HASHCHECK,
            .hierarchy = TPM2_RH_NULL,
            .digest = init_empty<TPM2B_DIGEST>(),
         });

         hash()->final(as_span(*result.first));
         return result;
      }
   }();
...
bool Verification_Operation::is_valid_signature(std::span<const uint8_t> sig_data) {
   auto digest = init_with_size<TPM2B_DIGEST>(hash()->output_length());
   hash()->final(as_span(digest));
