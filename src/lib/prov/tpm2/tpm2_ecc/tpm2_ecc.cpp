...
class EC_Signature_Operation final : public Signature_Operation {
   ...
   private:
      std::vector<uint8_t> marshal_signature(const TPMT_SIGNATURE& signature) const override {
         ...
         const auto half_sig_len = signature_length_for_key_handle(sessions(), key_handle()) / 2;
         BOTAN_ASSERT_NOMSG(r.size == half_sig_len && s.size == half_sig_len);

         return concat<std::vector<uint8_t>>(as_span(r), as_span(s));
      }
};
...
class EC_Verification_Operation final : public Verification_Operation {
   ...
   private:
      TPMT_SIGNATURE unmarshal_signature(std::span<const uint8_t> sig_data) const override {
         ...
         auto sig_slicer = BufferSlicer(sig_data);
         copy_into(sig.signature.ecdsa.signatureR, sig_slicer.take(sig_data.size() / 2));
         copy_into(sig.signature.ecdsa.signatureS, sig_slicer.take(sig_data.size() / 2));
         BOTAN_DEBUG_ASSERT(sig_slicer.empty());
