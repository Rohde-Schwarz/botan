...
class Ounsworth_Kat_Test final : public Text_Based_Test {
   public:
      ...
      Test::Result run_one_test(const std::string& /*params*/, const VarMap& vars) override {
         ...
         Botan::Sp800_56C::kdm(kdm_output, *mac, kdf_input, fixed_info, big_k_bytes);
...
class SP800_56C_One_Step_Kdm_Test final : public Text_Based_Test {
   public:
      SP800_56C_One_Step_Kdm_Test() : Text_Based_Test("pubkey/sp800_56c_one_step_kdm.vec", "Z,Salt,FixedInfo,Output") {}

      Test::Result run_one_test(const std::string& h_id, const VarMap& vars) override {
         Test::Result result("SP.800-56C One Step KDM Test");

         auto z = vars.get_req_bin("Z");
         auto salt = vars.get_req_bin("Salt");
         auto fixed_info = vars.get_req_bin("FixedInfo");
         auto output_ref = vars.get_req_bin("Output");

         std::vector<uint8_t> kdm_output(output_ref.size());
         if(auto hash = Botan::HashFunction::create(h_id)) {
            Botan::Sp800_56C::kdm(kdm_output, *hash, z, fixed_info);
         } else if(auto mac = Botan::MessageAuthenticationCode::create(h_id)) {
            Botan::Sp800_56C::kdm(kdm_output, *mac, z, fixed_info, salt);
         } else {
            result.note_missing("Hash/MAC function " + h_id);
            return result;
         }

         result.test_eq("KDM output", kdm_output, output_ref);

         return result;
      }
};

BOTAN_REGISTER_TEST("ounsworth", "ounsworth_sp800_56c_one_step_kdm", SP800_56C_One_Step_Kdm_Test);
